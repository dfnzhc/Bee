...
[[nodiscard]] auto parallel_reduce(ThreadPool& pool, It first, It last, T init, BinaryOp op) -> T
{
...
/// 带取消支持的并行归约。
/// @note 取消时，部分结果被丢弃并抛出异常。
template <typename It, typename T, typename BinaryOp>
[[nodiscard]] auto parallel_reduce(ThreadPool& pool, It first, It last, T init, BinaryOp op, std::stop_token token) -> T
{
    const auto n = static_cast<size_t>(std::distance(first, last));
    if (n == 0)
        return init;
...
[[nodiscard]] auto parallel_transform_reduce(ThreadPool& pool, It first, It last, T init, ReduceOp reduce_op, TransformOp transform_op) -> T
{
    const auto n = static_cast<size_t>(std::distance(first, last));
    if (n == 0)
        return init;
