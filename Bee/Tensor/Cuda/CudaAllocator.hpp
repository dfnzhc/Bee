...
// CUDA 分配类型。Device 表示普通张量持有的设备内存；Workspace 表示由
// CUDA 运行时统一管理、调用方只借用指针的工作区。
enum class CudaAllocKind : std::uint8_t
{
    Device,    // 常规 CUDA 设备内存
    Workspace, // 运行时管理的工作区内存
};
...
{
    // 注意：通过 allocate_for(CudaAllocKind::Workspace, ...) 获取的指针由运行时持有，
