...
namespace
{
    ...
    ) -> void
    {
        for (int64_t r = 0; r < n_rows; ++r) {
            ...
            // 归一化并乘权重
            for (int64_t i = 0; i < d; ++i) {
                row_out[i] = static_cast<T>(
                    static_cast<double>(row_in[i]) * rms_inv * static_cast<double>(w_ptr[i])
                );
            }
        }
    }
...
auto rms_norm(const Tensor& x, const Tensor& weight, double eps, const tensor::cuda::ExecContext* /*ctx*/) -> Result<Tensor>
{
    ...
    if (x.dtype() != DType::F32 && x.dtype() != DType::F64)
        return std::unexpected(make_error(
            std::format("rms_norm: x dtype 须为 F32 或 F64，当前 {}", enum_to_name(x.dtype())),
            Severity::Recoverable
        ));

    if (weight.dtype() != x.dtype())
        return std::unexpected(make_error(
            std::format("rms_norm: weight dtype（{}）须与 x dtype（{}）一致",
                        enum_to_name(weight.dtype()), enum_to_name(x.dtype())),
            Severity::Recoverable
        ));
    ...
    if (weight.ndim() != 1)
        return std::unexpected(make_error(
            std::format("rms_norm: weight 须为 1-D 张量，当前 ndim={}", weight.ndim()),
            Severity::Recoverable
        ));
