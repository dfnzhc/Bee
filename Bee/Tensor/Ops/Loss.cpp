    ...
    for (size_t i = 0; i < batch_size; ++i) {
        ...
        if (label < 0 || static_cast<size_t>(label) >= num_classes) {
            return std::unexpected(make_error(
                std::format("cross_entropy: target[{}] = {} 超出类别范围 [0, {})", i, label, num_classes),
...
auto cross_entropy(const Tensor& logits, const Tensor& target, const tensor::cuda::ExecContext* ctx) -> Result<Tensor>
{
    ...
    if (logits.dtype() != DType::F32 && logits.dtype() != DType::F64) {
        return std::unexpected(make_error(
            std::format("cross_entropy: 不支持的 logits dtype: {}", enum_to_name(logits.dtype())),
            Severity::Recoverable
        ));
    }
    ...
    if (target.dtype() != DType::I32 && target.dtype() != DType::I64) {
        return std::unexpected(make_error(
            std::format("cross_entropy: 不支持的 target dtype: {}", enum_to_name(target.dtype())),
            Severity::Recoverable
        ));
    }
    ...
    if (logits.ndim() != 2) {
        return std::unexpected(make_error(
            std::format("cross_entropy: logits 必须是 2D，实际维度: {}", logits.ndim()),
            Severity::Recoverable
        ));
    }
    ...
    if (target.ndim() != 1) {
        return std::unexpected(make_error(
            std::format("cross_entropy: target 必须是 1D，实际维度: {}", target.ndim()),
            Severity::Recoverable
        ));
    }
    ...
    if (N != target_N) {
        return std::unexpected(make_error(
            std::format("cross_entropy: logits 与 target batch 大小不匹配: {} vs {}", N, target_N),
            Severity::Recoverable
        ));
    }
    ...
    if (logits.device() != target.device()) {
        ...
        return std::unexpected(make_error(
            std::format("cross_entropy: logits 与 target 设备不匹配: {} vs {}", logits_dev, target_dev),
            Severity::Recoverable
        ));
    }
