...
inline void cast_simd_chunk<float, std::int32_t, simd::IsaSse2>(const float* s, std::int32_t* d, std::int64_t n)
{
    ...
    for (; i < n; ++i)
        d[i] = static_cast<std::int32_t>(s[i]);
}
...
inline void cpu_cast_linear_parallel(const Src* s, Dst* d, std::int64_t n)
{
    ...
    parallel::parallel_for(std::size_t{0}, static_cast<std::size_t>(n), static_cast<std::size_t>(grain), [&](std::size_t lo, std::size_t hi) {
...
#define BEE_CAST_CASE(SDT, DDT, STYPE, DTYPE)                                                                     \
    if (src_dt == DType::SDT && dst_dt == DType::DDT) {                                                           \
        cpu_cast_linear_parallel<STYPE, DTYPE, ISA>(static_cast<const STYPE*>(src), static_cast<DTYPE*>(dst), n); \
        return;                                                                                                   \
    }
