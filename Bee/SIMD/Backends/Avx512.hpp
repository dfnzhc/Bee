...
struct SimdBackend<float, IsaAvx512>
{
    ...
    // 绝对值：AVX-512F 原生指令
    static auto abs(reg a) -> reg { return _mm512_abs_ps(a); }

    static auto sqrt(reg a) -> reg { return _mm512_sqrt_ps(a); }
    ...
    static auto reduce_sum(reg v) -> float { return _mm512_reduce_add_ps(v); }

    // 水平求最小值
    static auto reduce_min(reg v) -> float { return _mm512_reduce_min_ps(v); }

    // 水平求最大值
    static auto reduce_max(reg v) -> float { return _mm512_reduce_max_ps(v); }
};
...
struct SimdBackend<double, IsaAvx512>
{
    ...
    static auto abs(reg a) -> reg { return _mm512_abs_pd(a); }

    static auto sqrt(reg a) -> reg { return _mm512_sqrt_pd(a); }
    ...
    static auto reduce_sum(reg v) -> double { return _mm512_reduce_add_pd(v); }
    static auto reduce_min(reg v) -> double { return _mm512_reduce_min_pd(v); }
    static auto reduce_max(reg v) -> double { return _mm512_reduce_max_pd(v); }
};
...
struct SimdBackend<int32_t, IsaAvx512>
{
    ...
    static auto set1(int32_t x) -> reg { return _mm512_set1_epi32(x); }

    static auto add(reg a, reg b) -> reg { return _mm512_add_epi32(a, b); }
    static auto sub(reg a, reg b) -> reg { return _mm512_sub_epi32(a, b); }

    // AVX-512F 原生 i32 乘法（低 32 位）
    static auto mul(reg a, reg b) -> reg { return _mm512_mullo_epi32(a, b); }

    static auto min(reg a, reg b) -> reg { return _mm512_min_epi32(a, b); }
    static auto max(reg a, reg b) -> reg { return _mm512_max_epi32(a, b); }

    // 取反：0 - v
    static auto neg(reg a) -> reg { return _mm512_sub_epi32(_mm512_setzero_si512(), a); }

    // AVX-512F 原生绝对值
    static auto abs(reg a) -> reg { return _mm512_abs_epi32(a); }
...
struct SimdBackend<int64_t, IsaAvx512>
{
    ...
    static auto set1(int64_t x) -> reg { return _mm512_set1_epi64(x); }

    static auto add(reg a, reg b) -> reg { return _mm512_add_epi64(a, b); }
    static auto sub(reg a, reg b) -> reg { return _mm512_sub_epi64(a, b); }

    // AVX-512F 原生 i64 min/max（AVX2 不支持）
    static auto min(reg a, reg b) -> reg { return _mm512_min_epi64(a, b); }
    static auto max(reg a, reg b) -> reg { return _mm512_max_epi64(a, b); }

    // 取反：0 - v
    static auto neg(reg a) -> reg { return _mm512_sub_epi64(_mm512_setzero_si512(), a); }

    // AVX-512F 原生 i64 绝对值（AVX2 不支持）
    static auto abs(reg a) -> reg { return _mm512_abs_epi64(a); }
    ...
    static auto reduce_min(reg v) -> int64_t
    {
        ...
        _mm512_store_si512(reinterpret_cast<__m512i*>(buf), v);
        int64_t r = buf[0];
        for (int i = 1; i < 8; ++i) r = r < buf[i] ? r : buf[i];
        return r;
    }
...
struct SimdBackend<uint8_t, IsaAvx512>
{
    ...
    static auto set1(uint8_t x) -> reg { return _mm512_set1_epi8(static_cast<char>(x)); }

    // 字节加法（环绕）
    static auto add(reg a, reg b) -> reg { return _mm512_add_epi8(a, b); }
    static auto sub(reg a, reg b) -> reg { return _mm512_sub_epi8(a, b); }

    // AVX512BW 无符号字节比较
    static auto min(reg a, reg b) -> reg { return _mm512_min_epu8(a, b); }
    static auto max(reg a, reg b) -> reg { return _mm512_max_epu8(a, b); }
