...
// 标量后端是所有运行时分派的保底实现，也是各 SIMD 后端尾部元素处理的
// 语义基准。每个寄存器只包含一个元素，所有 load/store 都退化为普通
// 标量访问；浮点数学函数直接调用 <cmath>，整数算术保持 C++ 对应类型
// 的环绕或截断行为。
...
// -----------------------------------------------------------------------
// uint8_t 标量后端
// 注意：不提供 neg/abs/mul/div。uint8_t 主要用于量化字节搬运与 min/max
// 规约，乘除和取负存在无符号溢出与提升规则争议，调用方应在更宽的累加
// 类型上实现这些语义。
// -----------------------------------------------------------------------
template <>
