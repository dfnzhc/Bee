...
void gemm_tma_wmma_tf32_kernel(
    ...
    auto issue_tma = [&] (int kt, int stage) {
        ...
        if (threadIdx.x == 0) {
            cde::cp_async_bulk_tensor_2d_global_to_shared(
                &stages[stage].A[0][0], &tmap_a, kt * BK, tile_y, bars[stage]);
            cde::cp_async_bulk_tensor_2d_global_to_shared(
                &stages[stage].B[0][0], &tmap_b, tile_x, kt * BK, bars[stage]);
            ::new (&token_at(stage)) barrier::arrival_token(
                ::cuda::device::barrier_arrive_tx(bars[stage], 1, kStageBytes));
        } else {
...
#pragma unroll
            for (int i = 0; i < FM_TILES; ++i) {
                load_matrix_sync(
                    a_frag[i],
                    &tile.A[warp_row * WM + i * FRAG_M][kf * FRAG_K],
                    BK);
#pragma unroll
...
#pragma unroll
            for (int j = 0; j < FN_TILES; ++j) {
                load_matrix_sync(
                    b_frag[j],
                    &tile.B[kf * FRAG_K][warp_col * WN + j * FRAG_N],
                    BN);
#pragma unroll
...
inline CUresult make_tensor_map_2d(CUtensorMap& out,
                                   const float*  base,
                                   std::uint64_t outer,
                                   std::uint64_t inner,
                                   std::uint32_t box_outer,
                                   std::uint32_t box_inner) noexcept
{
                            ...
                            cudaStream_t stream) noexcept
{
    ...
    if (auto r = make_tensor_map_2d(tmap_a, static_cast<const float*>(A),
    ...
    if (auto r = make_tensor_map_2d(tmap_b, static_cast<const float*>(B),
                                    static_cast<std::uint64_t>(K),
    ...
    gemm_tma_wmma_tf32_kernel<<<grid, block, smem_bytes, stream>>>(
        tmap_a, tmap_b, static_cast<float*>(C),
