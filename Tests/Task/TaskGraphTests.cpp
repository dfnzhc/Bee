...
TEST(TaskGraphTests, SimpleChain)
{
    ThreadPool pool(2);
    TaskGraph  graph;
    auto       a    = graph.add_node([] { return 10; });
    auto       b    = graph.add_node([](int x) { return x * 2; }, a);
    auto       c    = graph.add_node([](int x) { return x + 5; }, b);
    auto       task = graph.execute(pool);
    task.wait();
...
TEST(TaskGraphTests, DiamondDependency)
{
    ...
    TaskGraph  graph;
    auto       a    = graph.add_node([] { return 1; });
    auto       b    = graph.add_node([](int x) { return x + 10; }, a);
    auto       c    = graph.add_node([](int x) { return x * 100; }, a);
    auto       d    = graph.add_node([](int x, int y) { return x + y; }, b, c);
    auto       task = graph.execute(pool);
    task.wait();
...
TEST(TaskGraphTests, WideFanOut)
{
    ThreadPool                   pool(4);
    TaskGraph                    graph;
    auto                         root = graph.add_node([] { return 5; });
    std::vector<NodeHandle<int>> children;
    for (int i = 0; i < 10; ++i) {
        children.push_back(graph.add_node([i](int x) { return x + i; }, root));
    }
    auto task = graph.execute(pool);
    task.wait();
    EXPECT_EQ(task.state(), TaskState::Completed);
...
TEST(TaskGraphTests, TypedDataFlow)
{
    ...
    TaskGraph  graph;
    auto       a    = graph.add_node([] { return 42; });
    auto       b    = graph.add_node([](int x) { return std::to_string(x); }, a);
    auto       c    = graph.add_node([](const std::string& s) { return static_cast<double>(s.size()); }, b);
    auto       task = graph.execute(pool);
    task.wait();
    EXPECT_EQ(graph.result(a), 42);
...
TEST(TaskGraphTests, VoidNodesWithAddNodeAfter)
{
    ...
    auto             a    = graph.add_node([&] { counter.fetch_add(1, std::memory_order_relaxed); });
    auto             b    = graph.add_node_after([&] { counter.fetch_add(10, std::memory_order_relaxed); }, a);
    auto             task = graph.execute(pool);
    task.wait();
    EXPECT_EQ(task.state(), TaskState::Completed);
...
TEST(TaskGraphTests, WideFanIn)
{
    ...
    for (int i = 0; i < 10; ++i) {
        roots.push_back(graph.add_node([i] { return i; }));
    }

    std::atomic<int>              sum{0};
    std::vector<NodeHandle<void>> void_roots;
    for (auto& r : roots) {
        void_roots.push_back(graph.add_node([&sum](int x) { sum.fetch_add(x, std::memory_order_relaxed); }, r));
    }
...
TEST(TaskGraphTests, MixedVoidAndTyped)
{
    ...
    auto producer   = graph.add_node([] { return 10; });
    auto void_node  = graph.add_node([&] { side_effect.store(99, std::memory_order_relaxed); });
    auto consumer   = graph.add_node([](int x) { return x * 3; }, producer);
    auto final_node = graph.add_node_after([&] { return side_effect.load(std::memory_order_relaxed); }, void_node);
...
TEST(TaskGraphTests, ReExecution)
{
    ...
    auto             a = graph.add_node([&] { return call_count.fetch_add(1, std::memory_order_relaxed); });
