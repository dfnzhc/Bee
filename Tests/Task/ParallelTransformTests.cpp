...
TEST(ParallelTransformTests, BasicSquare)
{
    ...
    auto it = bee::parallel_transform(pool, input.begin(), input.end(), output.begin(),
                                      [](int x) {
                                          return x * x;
                                      });
...
TEST(ParallelTransformTests, TypeConversion)
{
    ...
    bee::parallel_transform(pool, input.begin(), input.end(), output.begin(),
                            [](int x) -> double {
...
TEST(ParallelTransformTests, ExceptionPropagation)
{
    ...
    EXPECT_THROW(
            bee::parallel_transform(pool, input.begin(), input.end(), output.begin(),
                [](int) -> int { throw std::runtime_error("transform error"); }),
            std::runtime_error);
}
...
TEST(ParallelTransformTests, Cancellation)
{
    ...
    EXPECT_THROW(
            bee::parallel_transform(pool, input.begin(), input.end(), output.begin(),
                [](int x) { return x * 2; }, source.get_token()),
            std::runtime_error);
}
