...
TEST(WhenAnyTests, BasicFirstWins)
{
    ...
    tasks.push_back(bee::submit(pool, [] { return 42; }, source.get_token()));
    for (int i = 0; i < 2; ++i) {
        tasks.push_back(
...
TEST(WhenAnyTests, SingleTask)
{
    ...
    std::vector<Task<int>> tasks;
    tasks.push_back(bee::submit(pool, [] { return 77; }, source.get_token()));
...
TEST(WhenAnyTests, FirstFailurePropagates)
{
    ...
    std::vector<Task<int>> tasks;
    tasks.push_back(bee::submit(pool, []() -> int { throw std::runtime_error("boom"); }, source.get_token()));
    for (int i = 0; i < 2; ++i) {
        tasks.push_back(
            bee::submit_cancellable(
                pool,
...
TEST(WhenAnyTests, AllExternallyCancelled)
{
    ...
    std::vector<Task<int>> tasks;
    for (int i = 0; i < 3; ++i) {
        tasks.push_back(bee::submit(pool, [i] { return i; }, source.get_token()));
    }
...
TEST(WhenAnyTests, CompletionAndFailureRace)
{
    ...
    std::vector<Task<int>> tasks;
    tasks.push_back(bee::submit(pool, [] { return 1; }, source.get_token()));
    tasks.push_back(bee::submit(pool, []() -> int { throw std::runtime_error("err"); }, source.get_token()));
...
TEST(WhenAnyTests, VectorLargeCount)
{
    ...
    std::vector<Task<int>> tasks;
    tasks.push_back(bee::submit(pool, [] { return 999; }, source.get_token()));
    for (int i = 0; i < 99; ++i) {
        tasks.push_back(
            bee::submit_cancellable(
                pool,
...
TEST(WhenAnyTests, VariadicHeterogeneousTypes)
{
    ...
    auto t1 = bee::submit(pool, [] { return 42; }, source.get_token());
...
TEST(WhenAnyTests, VariadicSingleTask)
{
    ...
    auto t1       = bee::submit(pool, [] { return 55; }, source.get_token());
...
TEST(WhenAnyTests, VariadicVoidTask)
{
    ...
    auto t1 = bee::submit(pool, [&executed] { executed.store(true); }, source.get_token());
    auto t2 = bee::submit_cancellable(
        pool,
        [](std::stop_token token) -> int {
            while (!token.stop_requested())
