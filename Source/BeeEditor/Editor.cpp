...
namespace
{
    ...
    const std::string MODEL_PATH   = std::string(EDITOR_SOURCE_DIR) + "/Dirty/viking_room.obj";
    const std::string TEXTURE_PATH = std::string(EDITOR_SOURCE_DIR) + "/Dirty/viking_room.png";
...
namespace
{
    ...
    public:
        ...
        {
            auto vertShaderCode = readFile(std::string(EDITOR_SOURCE_DIR) + "/Dirty/vert.spv");
            auto fragShaderCode = readFile(std::string(EDITOR_SOURCE_DIR) + "/Dirty/frag.spv");
    ...
    std::unique_ptr<HelloTriangleApplication> app = nullptr;
} // namespace 
...
{
    app = std::make_unique<HelloTriangleApplication>();
    if (!app)
        return false;
